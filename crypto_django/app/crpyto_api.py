...
def all_data():
    ...
    for i in range(len(data)):
        crypto_obj = {
            ...
            "price": int(data[i]['trade_price']) if data[i]['trade_price'] % 1 == 0 else data[i]['trade_price'],
            "change": data[i]['change'],
            "change_rate": float(data[i]['change_rate']),
            "change_price": int(data[i]['change_price']) if data[i]['change_price'] % 1 == 0 else data[i]['change_price'],
