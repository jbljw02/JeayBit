...
export default function ChartHeader() {
    ...
    const requestCandleMinute = useCallback(async (market: string, minute: string) => {
        if (minute && market) {
            try {
                const response = await axios.get(`https://jeaybit-daphne-server-502f69a34a39.herokuapp.com/candle_per_minute/?market=${market}&minute=${minute}`);
    ...
    const requestCandleDate = useCallback(async (market: string) => {
        try {
            let response;
            let url = "https://jeaybit-daphne-server-502f69a34a39.herokuapp.com/";
