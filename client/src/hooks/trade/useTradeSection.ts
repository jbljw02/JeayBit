...
export default function useTradeSection({ tradeType, price, setPrice }: UseTradeSectionProps) {
    ...
    const selectPercentage = (percentage: string) => {
        ...
        // 매도 주문
            ...
            // 보유중인 화폐 찾기
            const targetCrypto = ownedCrypto.find((crypto: Crypto) => crypto.name === selectedCrypto.name);
            // 매도 가능 수량
